...
import { enqueueSnifferRawEvent } from '../telemetry/snifferHook.js';
...
export class NetworkSniffer {
   ...
   * @param {{
   *   debug?: boolean;
   *   onDebugSample?: (e: Record<string, unknown>) => void;
   *   debugLogBudget?: number;
   *   rawEventBuffer?: import('../telemetry/rawEventBuffer.js').RawEventWriteBuffer | null;
   *   rawEventExtractorVersion?: string;
   *   runId?: string | null;
   *   sessionEpoch?: number | null;
   *   rawEventSampleRate?: number;
   * }} [opts]
   */
  constructor(page, urlIncludes, opts = {}) {
    ...
    /** @type {import('../telemetry/rawEventBuffer.js').RawEventWriteBuffer | null} */
    this._rawEventBuffer = opts.rawEventBuffer ?? null;
    this._rawEventExtractorVersion = opts.rawEventExtractorVersion ?? 'productExtract@v1';
    this._runId = opts.runId ?? null;
    this._sessionEpoch = opts.sessionEpoch ?? null;
    this._rawEventSampleRate = Number.isFinite(opts.rawEventSampleRate)
      ? opts.rawEventSampleRate
      : undefined;
  }
  ...
  async _onResponse(response) {
    try {
      ...
      enqueueSnifferRawEvent(this._rawEventBuffer, {
        method,
        url,
        httpStatus: status,
        bodyText: rawText,
        ...
        sampleRate: this._rawEventSampleRate,
        extractorVersion: this._rawEventExtractorVersion,
      });

      if (readError) return;
      if (status < 200 || status >= 300) return;

      const text = typeof rawText === 'string' ? stripJsonPrefix(rawText) : '';
